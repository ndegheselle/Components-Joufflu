...
- Add `Autocomplete`, a text box completing what is typed with the most likely item of its `ItemsSource` : the rest of that item is drawn greyed right after the caret, `Tab` takes it, and the items starting with what is typed are listed under the box, each drawn with the `ItemTemplate` given to the control. The text of an item is read from `TextMemberPath`, or from its `ToString()`, so what is completed on is not tied to what the list shows. Nothing forces the text into the list, which is the point : `SelectedItem` holds the item whose text was typed and the typed `string` itself when the list holds nothing spelled that way, so a value that is not one of the suggestions is a value all the same. `Down` and `Up` open the list and walk it with the completion following the highlight, `Enter` takes the highlighted suggestion, `Escape` closes the list without touching the text, and leaving the box only spells the text the way the list does when the list holds it
